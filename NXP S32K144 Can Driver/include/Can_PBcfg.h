...
typedef struct
{
	...
	/* We will consider it as the ID that we will put in the receiving message buffer */
...
typedef struct
{
	...
	/* The user must configure the IDs from 0 for each controller */ 
	Can_ObjectId CanObjectId;   /* Holds the number of each buffer */ 
